...
with FileUploadSupport with Metrics {
  ...
  post("/v1/regions/shapefile") {
    val friendlyName = params.getOrElse("friendlyName", halt(BadRequest("No friendlyName param provided")))
    val forceLonLat = Try(params.getOrElse("forceLonLat", "false").toBoolean)
                         .getOrElse(halt(BadRequest("Invalid forceLonLat param provided")))
    // TODO fileParams.get currently blows up if no post params are provided. Handle that scenario more gracefully.
    val file = fileParams.getOrElse("file", halt(BadRequest("No file param provided")))
  ...
  post("/v1/regions/:resourceName/stringcode") {
    ...
    if (strings.isEmpty) halt(GeospaceServlet.HttpClientError,
