      ...
      case Success((features, schema)) =>
        val ingressResult =
          ...
          yield {
            ...
            logger.info(s"Ingressed shapefile '$friendlyName' to resource name '${response.resourceName}' " +
                        s"(${response.upsertCount} rows, ${System.currentTimeMillis - startTime} milliseconds)");
