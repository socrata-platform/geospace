...
object FeatureValidator extends Logging {
  ...
  def validate(feature: Feature, maxMultiPolygonComplexity: Int): Result = {
    ...
    Option(rf.geometry) match {
      case Some(mp: MultiPolygon) => validateMultiPolygon(mp, maxMultiPolygonComplexity)
   ...
   * @param mp
   * @param maxMultiPolygonComplexity
   * @return
   */
  ...
  private def offTheMapPoints(coordinates: Array[Coordinate]): Result ={
  ...
  /**
   * Compare geometry complexity with max in configs.
   * @param complexity
   * @param maxComplexity
   * @return
   */
