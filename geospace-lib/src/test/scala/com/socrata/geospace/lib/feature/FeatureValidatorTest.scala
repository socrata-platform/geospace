...
class FeatureValidatorTest extends FunSuite with Matchers {
  ...
  def buildSimpleMultiPolygon(ring: Seq[(Double, Double)]): jts.MultiPolygon = builder.multi(Seq(builder.Polygon(ring)))

  test("Feature geometry is null") {
    val feature = featureBuilder.buildFeature(null, Array(null, "a nonexistent shape")) // scalastyle:ignore null
  ...
  test("Feature geometry is not a multipolygon") {
    ...
    val feature = pointBuilder.buildFeature("p", Array(point, "oops! this is a point not a multipolygon"))
    FeatureValidator.validate(feature, 5) should be(GeometryNotAMultiPolygon)
  }

  test("Feature geometry is not valid") {
    val mp = buildSimpleMultiPolygon(Seq(
      (-122.315972, 47.617889),
      (-122.322481, 47.611165),
      (-122.322192, 47.615205),
      (-122.310648, 47.615321),
      (-122.315972, 47.617889)))
    val feature = featureBuilder.buildFeature("mp", Array(mp, "Self-crossing polygon"))
  ...
  test("Feature geometry contains one or more coordinates that can't be displayed on a world map") {
    val mp = buildSimpleMultiPolygon(Seq((-122.315972, 47.617889),
      ...
      (-122.322192, 47.615205),
      (-122.315972, 47.617889)))
    val feature = featureBuilder.buildFeature("mp", Array(mp, "Polygon with an unmappable point"))

    val result = FeatureValidator.validate(feature, 5)
    result shouldBe a[GeometryContainsOffMapPoints]
    result.asInstanceOf[GeometryContainsOffMapPoints].pts should be(Array(new jts.Coordinate(-1122.322481, 47.611165)))
  }
  ...
  test("Feature geometry contains one or more coordinates on the world map boundary") {
    val mp = buildSimpleMultiPolygon(Seq(
      ...
      (-180.0, -90.0),
      (-180.0, 90.0)))
    val feature = featureBuilder.buildFeature("mp", Array(mp, "Polygon with points on the boundary"))
    FeatureValidator.validate(feature, 5) should be(Valid)
  }

  test("Feature geometry contains one or more coordinates on the world map boundary - allow reprojection rounding") {
    val mp = buildSimpleMultiPolygon(Seq(
      (180.0000009, 90.0000009),
      (-180.0000009, 90.0000009),
      (-180.0000009, -90.0000009),
      (180.0000009, -90.0000009),
      (180.0000009, 90.0000009)))
    val feature = featureBuilder.buildFeature("mp",
      Array(mp, "Polygon with points over the boundary by a rounding error"))
    FeatureValidator.validate(feature, 5) should be(Valid)
  }
  ...
  test("Feature geometry is too complex") {
    val mp = buildSimpleMultiPolygon(Seq(
      ...
      (-122.312592, 47.628404)))
    val feature = featureBuilder.buildFeature("mp", Array(mp, "Overly complex multipolygon"))
  ...
  test("Valid feature, single polygon") {
    val mp = buildSimpleMultiPolygon(Seq(
      ...
      (-122.322192, 47.615205),
      (-122.315972, 47.617889)))
    val feature = featureBuilder.buildFeature("mp", Array(mp, "A perfectly valid multipolygon!"))
    FeatureValidator.validate(feature, 5) should be(Valid)
  }
