...
import com.socrata.geospace.lib.client.SodaResponse.UnexpectedResponseCode
...
class CuratedRegionIndexerSpec extends FakeSodaFountain with Matchers {
  ...
  def schemaUrlPattern(resourceName: String): String = s"/dataset/$resourceName"

  def resourceUrlPattern(resourceName: String): String = s"/resource/$resourceName??.*"

  val validSchemaResponse =
    """{
      |resource_name:"_w2bc-s6ug",
      |name: "Chicago Zipcodes",
      |description:"",
      |row_identifier:":id",
      |locale:"en_US",
      |stage:"Unpublished"
      |}""".stripMargin
  val validGetResponse =
    """[{"bounding_multipolygon":{"type":"MultiPolygon","coordinates":[[[[0,0],[1,0],[1,1],[0,0]]]]}}]"""
  val validUpsertResponse = """[{"typ":"insert","id":"row-zgwi_rwzy~9aaf","ver":"rv-ju24.5fj2~wppc"}]"""

  def notFoundResponse(resourceName: String): String =
